...
export class CalCoService {
  ...
  private handlerMap = new Map([
    [CalcomBookingStatus.BOOKING_CREATED, this.bookingCreatedHandler],
    [CalcomBookingStatus.BOOKING_RESCHEDULED, this.bookingResheduledHandler],
  ]);
  ...
  private async bookingResheduledHandler(ctx: RequestContext, booking: Booking) {
    const eventUrls = this.generateEventUrl(booking);
    // const  cff = await this.transactionalConnection.rawConnection.getRepository(OrderLine).findOne({
    //   where: {
    //     eventUrls,
    //   }
    // })
    // if(!(cff instanceof CustomFulfillmentFields)) {
    //   throw new Error("ccc")
    // }
    // cff.
  ...
  generateEventUrl(booking: Booking) {
    return `${booking.payload.bookerUrl}/booking/${booking.payload.uid}`;
}
